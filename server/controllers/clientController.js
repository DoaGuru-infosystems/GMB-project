...
exports.getClientProfile = (req, res) => {
  ...
  db.query(
    "SELECT id, clientId, name, businessName, keywords, email, mobile, placeId, logo, isActive, primaryColor, secondaryColor, questions FROM clients WHERE clientId = ?",
    [clientId],
    (err, results) => {
      if (err) {
        console.error(err);
...
exports.updateClientProfile = (req, res) => {
  ...
  db.query(
    query,
    [name, businessName, mobile, logo, keywords, placeId, clientId],
    (err) => {
      if (err) {
        console.error(err);
...
exports.getClientReviews = (req, res) => {
  ...
  if (dateRange) {
    if (dateRange === "Custom Range" && startDate && endDate) {
      ...
      query +=
        " AND createdAt >= DATE_SUB(DATE_FORMAT(NOW() ,'%Y-%m-01'), INTERVAL 1 MONTH) AND createdAt < DATE_FORMAT(NOW() ,'%Y-%m-01')";
...
exports.markClientNotificationRead = (req, res) => {
  ...
  const clientId = req.user.clientId || req.user.clientID;
  db.query(
    "UPDATE notifications SET is_read = 1 WHERE id = ? AND clientId = ?",
    [id, clientId],
    (err) => {
      if (err) {
...
exports.getPublicClientProfile = (req, res) => {
  ...
  db.query(
    "SELECT businessName, keywords, logo, primaryColor, secondaryColor, questions FROM clients WHERE clientId = ?",
    [clientId],
    (err, results) => {
      if (err) {
        console.error(err);
