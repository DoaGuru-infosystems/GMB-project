      ...
      (err) => {
        if (err) {
          ...
          if (err.code === "ER_DUP_ENTRY") {
            return res
              .status(400)
              .json({ message: "Client with this email or ID already exists" });
          }
          return res
            .status(500)
            .json({ message: "Error creating client: " + err.message });
        }
  ...
  db.query("SELECT * FROM clients ORDER BY createdAt DESC", (err, result) => {
    if (err) {
      ...
      return res.status(500).json({
        message: "Error fetching clients: " + err.message,
        sqlError: err,
      });
    }
    ...
    const safeClients = result.map((client) => {
      ...
      if (safeData.keywords && typeof safeData.keywords === "string") {
        try {
          safeData.keywords = JSON.parse(safeData.keywords);
        } catch (e) {
          safeData.keywords = [];
        }
      }
...
exports.updateClient = async (req, res) => {
  ...
  try {
    ...
    if (password) {
      const hashed = await bcrypt.hash(password, 10);
      query =
        "UPDATE clients SET name=?, businessName=?, keywords=?, email=?, mobile=?, password=?, placeId=?, logo=?, websiteUrl=?, primaryColor=?, secondaryColor=?, questions=? WHERE clientId=?";
      params = [
        name,
        businessName,
        keywordsJson,
...
exports.getExpiringSubscriptions = (req, res) => {
  ...
  db.query(query, [daysAhead], (err, result) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Error fetching expiring subscriptions" });
    }
    res.json(result);
  });
};
